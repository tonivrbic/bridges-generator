...
function removeBridges(puzzle) {
    ...
    newPuzzle.forEach((row, i) => row.forEach((field, j) => {
        switch (field) {
            case "-":
            case "=":
            case "|":
...
const generate = (rows, columns, numberOfIslands, doubleBridges = 0.25) => {
    ...
    while (puzzleGenerated === false) {
        ...
        let islands = [];
        let bridges = [];
        puzzleGenerated = fillPuzzleWithIslands(rows, columns, islands, numberOfIslands, puzzle, bridges);
...
function isPuzzleSolvable(puzzle) {
    ...
    return bridgesSolver.solver(emptyPuzzle, 2, true);
}
function isPuzzleSolvableAgain(puzzle) {
    let emptyPuzzle = removeBridges(puzzle);
    let result = bridgesSolver.solver(emptyPuzzle, 3, true);
...
function fillPuzzleWithIslands(rows, columns, islands, numberOfIslands, puzzle, bridges) {
    ...
    while (islandsRemaining > 0) {
        ...
        switch (direction) {
            case 0:
                ...
                break;
            case 1:
                generated = tryGeneratingNaighborRight(puzzle, selected, islands, bridges, columns);
                break;
            case 2:
                generated = tryGeneratingNaighborDown(puzzle, selected, islands, bridges, rows);
                break;
            case 3:
                generated = tryGeneratingNaighborLeft(puzzle, selected, islands, bridges);
                break;
            default:
                break;
        }
...
function tryGeneratingNaighborUp(puzzle, selected, islands, bridges) {
    let desiredLocation = Math.max(Math.floor(Math.random() * (selected.row - 2)), 0);
...
function tryGeneratingNaighborRight(puzzle, selected, islands, bridges, columns) {
    let desiredLocation = Math.min(Math.floor(selected.col + 2 + Math.random() * (columns - selected.col - 2)), columns - 1);
    let bridgeDetected = false;
...
function tryGeneratingNaighborDown(puzzle, selected, islands, bridges, rows) {
    let desiredLocation = Math.min(Math.floor(selected.row + 2 + Math.random() * (rows - selected.row - 2)), rows - 1);
    let bridgeDetected = false;
    if (selected.row + 2 > desiredLocation) {
        return false;
    }
...
function tryGeneratingNaighborLeft(puzzle, selected, islands, bridges) {
    let desiredLocation = Math.max(Math.floor(Math.random() * (selected.col - 2)), 0);
    let bridgeDetected = false;
    if (selected.col - 2 < desiredLocation) {
        return false;
    }
